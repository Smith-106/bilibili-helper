...
var U1=async(e,n)=>{let o=[],t=1,r=30,l=1/0,g=!1;for(;;){if(bx0.cancel)break;let f=await bW({keyword:"",mid:e,order:"pubdate",order_avoided:"true",platform:"web",pn:t,ps:r,tid:0,web_location:"333.1387",special_type:"",index:0,...bDm()}),u=await(await window.fetch("//api.bilibili.com/x/space/wbi/arc/search?"+f,V)).json();if(u.code!==E.OK){if(u.code===-352)throw new Error("获取UP主视频列表失败：风控校验失败（-352），请稍候再试，切勿频繁重试");if((u.code===-403||u.code===-401)&&!g){g=!0,bWk=null,bWt=0,n&&n(" 签名已过期，正在重算后重试…"),await bx2(1500+Math.floor(Math.random()*500));continue}if(u.code===-403||u.code===-401)throw new Error("获取UP主视频列表失败：访问权限不足（"+u.code+"），已重算签名仍失败，请确认已登录B站并稍候再试");throw new Error("获取UP主视频列表失败："+(u.message||u.code))}let c=u.data&&u.data.list&&u.data.list.vlist||[];if(!c.length)break;c.forEach(i=>o.push({aid:i.aid,bvid:i.bvid,title:i.title,cid:0})),l=u.data&&u.data.page&&u.data.page.total||o.length,n&&n(` 已获取${o.length}/${l}个…`);if(o.length>=l||t>=40)break;t++,await bx2(800+Math.floor(Math.random()*400))}return o};
