...
var U1=async(e,n)=>{let o=[],t=1,r=30,l=1/0,g=!1,q=!1;let tp=async()=>{try{let u=await(await window.fetch("//api.bilibili.com/x/space/top/arc?vmid="+e,V)).json(),d=u.data;if(u.code===E.OK&&d&&(d.bvid||d.aid))return[{aid:d.aid,bvid:d.bvid,title:d.title||("av"+d.aid),cid:d.cid||0}]}catch(_){}return[]};for(;;){if(bx0.cancel){if(o.length)q=!0;break}let u=null;try{let f=await bW({keyword:"",mid:e,order:"pubdate",order_avoided:"true",platform:"web",pn:t,ps:r,tid:0,web_location:"333.1387",special_type:"",index:0,...bDm()});u=await(await window.fetch("//api.bilibili.com/x/space/wbi/arc/search?"+f,V)).json()}catch(_){u=null}if(!u||u.code!==E.OK){let c=u&&u.code;if((c===-403||c===-401)&&!g){g=!0,bWk=null,bWt=0,n&&n(" 签名已过期，正在重算后重试…"),await bx2(1500+Math.floor(Math.random()*500));continue}if(o.length){q=!0;break}let k=await tp();if(k.length){o=k,q=!0;break}if(c===-352)throw new Error("获取UP主视频列表失败：风控校验失败（-352），请稍候再试，切勿频繁重试");if(c===-403||c===-401)throw new Error("获取UP主视频列表失败：访问权限不足（"+c+"），已重算签名仍失败，请确认已登录B站并稍候再试");if(c===-799)throw new Error("获取UP主视频列表失败：请求过于频繁（-799），请休息几分钟再试，切勿连续重试");throw new Error("获取UP主视频列表失败："+((u&&u.message)||c||"网络异常"))}let c=u.data&&u.data.list&&u.data.list.vlist||[];if(!c.length)break;c.forEach(i=>o.push({aid:i.aid,bvid:i.bvid,title:i.title,cid:0})),l=u.data&&u.data.page&&u.data.page.total||o.length,n&&n(` 已获取${o.length}/${l}个…`);if(o.length>=l||t>=40)break;t++,await bx2(800+Math.floor(Math.random()*400))}o.partial=q;return o};
