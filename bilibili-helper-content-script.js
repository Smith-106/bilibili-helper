...
var Dx=async e=>{
  ...
  let i=e.getElementById("batch-btn"),d=e.getElementById("batch-cancel"),o=e.getElementById("batch-status"),r=e.getElementById("batch-progress"),u=e.getElementById("batch-skip"),c=window.__INITIAL_STATE__,f=c&&c.videoData&&c.videoData.ugc_season,P=f&&f.id||"unknown",Q0=0,Q1=0;
  ...
  for(let a=0;a<t.length;a++){
    ...
    if(u&&u.checked&&bx5(P).includes(h.cid)){Q1++,p.classList.add("b-skip"),p.innerHTML=`<span class="b-ep" title="${esc(m)}">${a+1}/${t.length} ${esc(m)}</span> ↷ 上次已下载，跳过`,o.textContent=` 正在处理 ${a+1}/${t.length}（成功${Q0} 失败${Q1}）`,bx0.lastStatus=o.textContent;continue}
    o.textContent=` 正在处理 ${a+1}/${t.length}（成功${Q0} 失败${Q1}）`,bx0.lastStatus=o.textContent;
    try{
      ...
      }else if(N.durl&&N.durl.length===1)await R(p,{url:N.durl[0].url,size:N.durl[0].size},`${String(a+1).padStart(3,"0")}_${m}`,p),Q0++,p.classList.add("b-ok"),p.innerHTML=`<span class="b-ep" title="${esc(m)}">${a+1}/${t.length} ${esc(m)}</span> ✔ 已保存`;
  ...
  if(bx0.cancel)o.textContent=` 已取消（成功${Q0} 失败${Q1}）`;
  else if(Q1===0)o.textContent=` 🎉 全部完成：成功${Q0} 失败0`,o.classList.add("b-celebrate"),setTimeout(()=>o.classList.remove("b-celebrate"),900);
  else o.textContent=` 全部完成：成功${Q0} 失败${Q1}（常见原因：未登录/未充电/视频不可见）`;
