          ...
          <div id="batchBox" style="display:none;">
            ...
            <div class="setting-item" style="margin-top:6px;">
              ...
              <label style="margin-left:14px;">速度：<select id="batch-speed" aria-label="批量下载速度" style="padding:2px 6px;border:1px solid #ccc;border-radius:3px;background:#fff;">
                ...
                <option value="18">超稳（列表也慢，适合被限流时）</option>
              </select></label>
            </div>
            <div class="setting-item" style="margin-top:6px;">
              <span class="label">UP主页：</span>
              <button id="up-btn" class="btn-ghost" style="padding:6px 12px;">批量下载该UP主全部视频</button>
              <div id="up-integrity" style="margin-top:4px;font-size:12px;color:#555;" aria-live="polite"></div>
            </div>
...
var bx0={running:!1,cancel:!1,lastStatus:"",failLog:[],lastUplist:null};
...
var bx8=(e,t)=>{try{let g=e.getElementById("up-integrity");if(!g)return;let got=t.length||0,ex2=t.expected||got,st=t.cool?"冷却中":t.cached?"缓存":t.partial?"部分":"完整",ct=t.cacheTs?new Date(t.cacheTs).toLocaleString():"—";g.innerHTML="";let s=document.createElement("span");s.textContent=` 完整性：预期${ex2} / 实际${got} / 缺${Math.max(0,ex2-got)} / ${st} / 缓存${ct}`;g.appendChild(s);if(t.partial||t.cached){let b=document.createElement("button");b.className="btn-ghost";b.style.cssText="padding:2px 8px;font-size:12px;margin-left:8px;";b.textContent="重抓完整列表";b.addEventListener("click",()=>Dv(e)),g.appendChild(b)}}catch(_){}};
var Dv=async e=>{if(bx0.running)return;let i=window.__INITIAL_STATE__,d=i&&i.videoData&&i.videoData.owner;if(!d||!d.mid)return v("未找到UP主信息");let o=e.getElementById("batch-status"),u=e.getElementById("batch-progress"),b=e.getElementById("up-btn");bx0.running=!0,bx0.cancel=!1,b&&(b.disabled=!0,b.classList.add("b-running")),o.textContent=" 正在获取UP主视频列表…",u.innerHTML="";try{let t=await U1(d.mid,n=>o.textContent=n);bx0.lastUplist={length:t.length,expected:t.expected||0,cached:!!t.cached,partial:!!t.partial,cool:!!t.cool,cacheTs:t.cacheTs||0},bx8(e,t);if(t.cool){let cw=t.coolWait||900;o.textContent=" 请求过于频繁，已进入冷却（剩"+Math.floor(cw/60)+"分"+(cw%60)+"秒），期间不再请求，稍候再试或用缓存继续…",bx0.running=!1,b&&(b.disabled=!1,b.classList.remove("b-running"));return}if(!t.length){o.textContent=" 未获取到视频列表（网络或风控拦截，稍候可再试）",bx0.running=!1,b&&(b.disabled=!1,b.classList.remove("b-running"));return}o.textContent=` 共${t.length}个视频${t.cached?"（"+Math.max(1,1)+"缓存列表，可能不是最新，稍候可刷新）":t.partial?"（部分列表，完整列表被风控拦截，稍候可再试）":""}，开始批量下载…`,await bxD(e,t,"up"+d.mid)}catch(n){o.textContent=" ✘ "+esc(String(n.message||n)),v("UP主批量下载失败：",n),bx0.running=!1,b&&(b.disabled=!1,b.classList.remove("b-running"))}};
var U1=async(e,n)=>{let o=[],t=1,r=30,l=1/0,q=!1,rt=0,ex=0,seen=new Set;const CK="bilibili_helper_uplist_"+e,CD="bilibili_helper_uplist_cool_"+e;const rd=(k,d)=>{try{let x=localStorage.getItem(k);return x?JSON.parse(x):d}catch(_){return d}};const wr=(k,v)=>{try{localStorage.setItem(k,JSON.stringify(v))}catch(_){}};let dv=3;try{dv=+(localStorage.getItem("bilibili_helper_batch_delay")||3)||3}catch(_){}let base=dv>=18?8500:dv>=12?7000:dv>=6?5500:4500;let cd0=rd(CD,0);if(cd0&&Date.now()-cd0<9e5){let w0=Math.ceil((9e5-(Date.now()-cd0))/1e3),cc0=rd(CK,null);if(cc0&&cc0.v&&cc0.v.length){o=cc0.v.map(i=>({aid:i[0],bvid:i[1],title:i[2],cid:0}));o.partial=!0;o.cached=!0;o.expected=cc0.count||o.length;o.cacheTs=cc0.ts||0;n&&n(` 风控冷却中(剩${Math.floor(w0/60)}分${w0%60}秒)，暂不请求，用${Math.max(1,Math.round((Date.now()-cc0.ts)/36e5))}小时前缓存${o.length}个继续…`);return o}let m0=[];m0.cool=!0;m0.coolWait=w0;m0.expected=0;m0.cacheTs=0;n&&n(` 请求过于频繁，冷却中(剩${Math.floor(w0/60)}分${w0%60}秒)，暂不发起请求，稍候再试…`);return m0}await bx2(2000+Math.floor(Math.random()*1000));for(;;){if(bx0.cancel){if(o.length)q=!0;break}let cdN=rd(CD,0);if(cdN&&Date.now()-cdN<9e5&&t>1){q=!0;break}let u=null;try{u=await(await window.fetch("//api.bilibili.com/x/space/arc/search?mid="+e+"&pn="+t+"&ps="+r+"&tid=0&keyword=&order=pubdate&jsonp=jsonp",V)).json()}catch(_){u=null}if(!u||u.code!==E.OK){let c=u&&u.code;if(c===-799||!u){if(!o.length&&t===1){if(rt<1){rt++,n&&n(` 请求受限，休息约12秒后重试…`),await bx2(10000+Math.floor(Math.random()*3000));continue}wr(CD,Date.now());let cc=rd(CK,null);if(cc&&cc.v&&cc.v.length){o=cc.v.map(i=>({aid:i[0],bvid:i[1],title:i[2],cid:0}));o.partial=!0;o.cached=!0;o.expected=cc.count||o.length;o.cacheTs=cc.ts||0;n&&n(` 实时列表被拦截，改用缓存${o.length}个继续…`);return o}let m=[];m.cool=!0;m.coolWait=900;m.expected=0;m.cacheTs=0;n&&n(` 请求过于频繁，已进入15分钟冷却(期间不再请求)，稍候再试…`);return m}ex=Math.min(6000,ex+2000);if(rt<4){rt++,n&&n(` 请求受限，休息${14+rt*6}秒后继续…(第${rt}次，已自动放慢)`),await bx2(14000+rt*6000+Math.floor(Math.random()*3000));continue}wr(CD,Date.now());if(o.length){q=!0;n&&n(` 已抓${o.length}/${l}个后持续受限，进入15分钟冷却(增量已缓存)，稍候可再试…`);break}q=!0;break}if(o.length){q=!0;break}q=!0;break}rt=0;let c=u.data&&u.data.list&&u.data.list.vlist||[];if(!c.length)break;let nd=0;c.forEach(i=>{if(i&&i.bvid&&!seen.has(i.bvid)){seen.add(i.bvid),o.push({aid:i.aid,bvid:i.bvid,title:i.title,cid:0}),nd++}}),l=(u.data&&u.data.page&&(u.data.page.count||u.data.page.total))||o.length;try{wr(CK,{ts:Date.now(),count:l,v:o.map(i=>[i.aid,i.bvid,i.title])})}catch(_){}n&&n(` 已获取${o.length}/${l}个…`);if(o.length>=l||t>=40)break;if(!nd)break;t++,await bx2(base+ex+Math.floor(Math.random()*1500))}if(!q){try{localStorage.removeItem(CD)}catch(_){}}o.expected=(l===1/0?o.length:l);o.cacheTs=Date.now();o.partial=q;return o};
