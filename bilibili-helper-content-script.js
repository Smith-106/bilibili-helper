...
var Dv=async e=>{if(bx0.running)return;let i=window.__INITIAL_STATE__,d=i&&i.videoData&&i.videoData.owner;if(!d||!d.mid)return v("未找到UP主信息");let o=e.getElementById("batch-status"),u=e.getElementById("batch-progress"),b=e.getElementById("up-btn");bx0.running=!0,b&&(b.disabled=!0,b.classList.add("b-running")),o.textContent=" 正在获取UP主视频列表…",u.innerHTML="";try{let t=await U1(d.mid,n=>o.textContent=n);if(!t.length){o.textContent=" 未获取到视频列表",bx0.running=!1,b&&(b.disabled=!1,b.classList.remove("b-running"));return}o.textContent=` 共${t.length}个视频，开始批量下载…`,await bxD(e,t,"up"+d.mid)}catch(n){o.textContent=" ✘ "+esc(String(n.message||n)),v("UP主批量下载失败：",n),bx0.running=!1,b&&(b.disabled=!1,b.classList.remove("b-running"))}};
var MIXIN=[46,47,18,2,53,8,23,32,15,50,10,31,58,3,45,35,27,43,5,49,33,9,34,4,14,24,12,38,41,13,37,48];
var bW=async e=>{let o=await(await window.fetch("//api.bilibili.com/x/web-interface/nav",V)).json(),i=o.data&&o.data.wbi_img||{},d=(i.img_url||"").split("/").pop().split(".")[0],a=(i.sub_url||"").split("/").pop().split(".")[0],r=(d+a).split(""),l=MIXIN.map(t=>r[t]).join("").slice(0,32),f={...e,wts:Math.floor(Date.now()/1000)},s=Object.keys(f).sort().map(t=>t+"="+encodeURIComponent(f[t])).join("&");return s+"&w_rid="+bM5(new TextEncoder().encode(s+l))};
...
var U1=async(e,n)=>{let o=[],t=1,r=25,l=1/0;for(;;){if(bx0.cancel)break;let f=await bW({mid:e,pn:t,ps:r,order:"pubdate","dm_img_list":"[]","dm_img_str":"V2Vi","dm_img_int":"0"}),u=await(await window.fetch("//api.bilibili.com/x/space/wbi/arc/search?"+f,V)).json();if(u.code!==E.OK)throw new Error("获取UP主视频列表失败："+(u.message||u.code));let c=u.data&&u.data.list&&u.data.list.vlist||[];if(!c.length)break;c.forEach(i=>o.push({aid:i.aid,bvid:i.bvid,title:i.title,cid:0})),l=u.data&&u.data.page&&u.data.page.total||o.length,n&&n(` 已获取${o.length}/${l}个…`);if(o.length>=l||t>=40)break;t++,await bx2(800)}return o};
