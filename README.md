...
**速度档**（`localStorage.bilibili_helper_batch_delay`，默认 3s/集）：快速 1s / 标准 3s / 保守 6s / 最稳 12s / 超稳 18s（被限流时用）。列表页间隔为类人 3.5–5s。越慢越不易触发风控。
...
| `-799`/网络异常退避 | 分两档：**pn1 双败 fast-fail**（首屏连续 2 次失败只重试 1 次约 12s，随即进 15min 冷却、期间零请求）；**中途分页**有界递增退避最多 4 次（约 12/16/20/24/27s）后继续 |
